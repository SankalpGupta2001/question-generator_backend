...
async function generateQuestionsDirectly(topic, questionType) {
  ...
  for (let i = 1; i <= 10; i++) {
    ...
    console.log(response, " The question is ",i ,"th question");
    // Extracting the answer and options based on question type
    let questionText="";
    let answer = "";
    let options = [];
    if (questionType === "mcq") {

      const responseString = response.toString(); // Convert response to a string
...
// Extract the answer along with additional information
...
answer = answerText.replace(/\*\*|\s+/g, ''); // Remove '**' and extra whitespaces
...
answer = answerText;
      ...
      console.log(options, " The option for question ", i, "is and questionText will be = ", questionText, " and the answer will be = ", answer);
    } else if (questionType === 'truefalse') {

      const responseString = response.toString(); // Convert response to a string

      // Find the index of relevant markers
      const questionStartIndex = responseString.indexOf('Question:');
      const answerStartIndex = responseString.indexOf('Answer:');
      ...
      // Extract the entire answer
      const answerText = responseString.substring(answerStartIndex + 'Answer:'.length).trim();
      answer=answerText;  

      console.log(" The question ", i, "is and questionText will be = ", questionText, " and the answer will be = ", answer);
